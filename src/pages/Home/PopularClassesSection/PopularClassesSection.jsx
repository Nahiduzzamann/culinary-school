    ...
    <div className="container mx-auto my-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {topClasses.map((classItem) => (
          <div key={classItem._id} className="bg-white rounded shadow overflow-hidden">
            <img src={classItem.classImage} alt='class image' className="w-full h-40 object-cover" />
            <div className="p-4">
              <h3 className="text-lg font-semibold mb-2">{classItem.className}</h3>
              <p className="text-gray-500">{classItem.students || '0'} students</p>
            </div>
          </div>
        ))}
      </div>
